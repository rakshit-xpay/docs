...
```mermaid
%%{init: {'theme': 'base', 'themeVariables': {'primaryColor': '#1E90FF', 'edgeLabelBackground': 'transparent', 'tertiaryColor': '#87CEEB', 'secondaryColor': '#4682B4', 'fontFamily': 'Arial', 'fontSize': 16}}}%%

graph TD
  subgraph Merchant Side
    A[Merchant Server] -- "Create Payment Intent" --> B[xPay Server]
    B -- "xIntentId" --> A
    A -- "xIntentId" --> C[Merchant Frontend]
    C -- "Redirect with xIntentId" --> D[xPay Client]
    D -- "Redirect with status and metadata" --> C
  end
